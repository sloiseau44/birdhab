...
- **`spring.datasource.hikari.max-lifetime: 180000` (+ `maximum-pool-size: 5`) dans les 6 `application.yml` backend, sous le délai de suspension de Neon.** Neon (Postgres serverless) suspend son instance après quelques minutes d'inactivité et ferme les connexions TCP existantes sans prévenir le pool ; le `max-lifetime` par défaut d'Hikari (30 min) laisse alors des connexions mortes dans le pool bien après leur fermeture côté Neon. Diagnostiqué via de vrais logs `birdhab-auth` en production : rafale de `Failed to validate connection ... this connection has been closed` sur les 10 connexions du pool, qui faisait échouer connexion/inscription avec des messages pris à tort pour de vrais rejets applicatifs (ex. "email ou mot de passe invalide" alors que le compte existait bien en base). Si ce symptôme réapparaît ailleurs (nouveau service, ou toujours après ce fix), vérifier d'abord les logs Hikari avant de soupçonner un bug applicatif.
...
- **`frontend/nginx.conf.template` : `proxy_ssl_server_name on` obligatoire pour un `GATEWAY_URL` en `https://`.** Nginx n'envoie pas le SNI par défaut vers un amont HTTPS proxifié ; Render héberge de nombreux services derrière des IP partagées différenciées par le SNI, donc sans cette directive la poignée de main TLS échoue (`SSL_do_handshake() failed`, 502 en façade). Invisible en local (Docker Compose : `GATEWAY_URL=http://gateway:8080`, sans TLS) — trouvé uniquement via un vrai déploiement Render, logs à l'appui. Header `Host` forwardé en `$proxy_host` (hôte de la Gateway), pas `$host` (hôte de la requête entrante) — sans impact fonctionnel actuel (la Gateway route uniquement par chemin, jamais par Host) mais sémantiquement correct pour un hôte externe différent.
- **429 "hibernate-rate-limited" de Render (réveil d'un service gratuit) : détecté via `isRateLimited` (`src/lib/errors.ts`, statut HTTP 429 — l'appli n'a aucune autre logique de rate-limit) sur `LoginPage`/`RegisterPage`, affiché avec un compte à rebours de 60s (`useCooldown`, `src/lib/useCooldown.ts`) qui désactive le bouton plutôt qu'un message d'échec.** Recliquer pendant le réveil prolonge le blocage côté Render ; le compte à rebours empêche ça physiquement plutôt que de compter sur la discipline de l'utilisateur. À reproduire sur toute autre page si le même 429 y est observé.
- **Réveil silencieux de la chaîne Gateway -> auth dès l'arrivée sur Login/RegisterPage (`useBackendWarmup`, `src/lib/useBackendWarmup.ts`).** Motivé par un vrai 502 observé en cascade : plusieurs services gratuits Render se réveillent indépendamment (frontend -> gateway -> auth), et soumettre le formulaire avant que toute la chaîne soit chaude produit un 502 générique Render même après avoir respecté le délai du 429 ci-dessus. Ping direct vers `/auth/me` en `axios.get` brut (pas `apiClient` : passer par l'intercepteur déclencherait sa logique de refresh sur le 401 attendu, masquant le vrai signal), toute réponse applicative (401 inclus) vaut "prêt" ; seuls 429/502/503/504/absence de réponse font réessayer (4s d'intervalle, 180s de plafond, `hasTimedOut` sinon avec message explicite sur la page plutôt qu'un déblocage silencieux). Bouton affiché "Préparation du service…" et désactivé pendant ce temps. **Le plafond initial de 60s s'est révélé trop court** : Gateway et auth cold-startent chacun jusqu'à ~2 min sur ce tier gratuit (log réel : `Started AuthServiceApplication in 119.901 seconds`), et Gateway doit être debout avant même de transmettre la requête à auth — pire cas proche de la somme des deux, pas du max. Avec 60s, le bouton se débloquait trop tôt et renvoyait l'utilisateur dans la boucle 429/compte-à-rebours du point précédent au lieu d'attendre le réveil complet. Porté à 180s.
- **Réveil silencieux de TOUS les services métier dès l'entrée dans l'appli authentifiée (`useServicesWarmup`, `src/lib/useServicesWarmup.ts`, appelé depuis `RequireAuth.tsx`).** Généralise `useBackendWarmup` ci-dessus : réveiller auth/gateway au login ne réveille pas `property`/`tenant`/`lease`/`payment`/`document`, qui dorment chacun indépendamment. Repéré via un vrai écran (page Locataires) bloqué sur "Chargement…" avec 17 erreurs 429/502 en console, une par requête, aggravées par les 3 relances automatiques par défaut de React Query sur chaque échec (un vrai orage de requêtes). `RequireAuth` (enveloppe toute l'app authentifiée, donc un seul réveil par session) ping en parallèle chaque chemin métier via `apiClient` (contrairement à `useBackendWarmup` : ici on est déjà authentifié, un 401 mérite la logique normale de refresh), retente tant qu'un chemin répond encore 429/502-504/erreur réseau (`isServiceUnavailable`, `src/lib/errors.ts`, factorisé depuis `useBackendWarmup`), prêt dès que tous ont répondu ou après 150s. `main.tsx` complète en désactivant les relances automatiques de React Query sur un 429 (`retry: (n, err) => n < 2 && !isRateLimited(err)`), pour ne pas concurrencer ce réveil proactif. **Le plafond initial de 60s était trop court** : ces 5 services n'avaient jamais été sollicités depuis le login (contrairement à auth/gateway), donc ce sont 5 JVM Spring Boot qui cold-startent en parallèle pour la première fois sur un tier gratuit à ressources partagées — passé à 150s, avec un état `hasTimedOut` distinct de `isWarmingUp` pour que `RequireAuth` affiche un message explicite + bouton "Réessayer" plutôt que de rendre une page dont les propres requêtes échoueraient à leur tour en silence (observé en usage réel : "Une erreur est survenue" générique sur la page Biens).
