...
- **Réveil silencieux de TOUS les services (auth/gateway + les 5 métier) dès l'arrivée sur Login/RegisterPage (`useBackendWarmup`, `src/lib/useBackendWarmup.ts`), avec `useServicesWarmup`/`RequireAuth.tsx` en filet de sécurité post-connexion.** Logique de retry/timeout partagée dans `src/lib/useWarmup.ts` (`isWarmingUp`/`hasTimedOut`, 15s d'intervalle, 180s de plafond), paramétrée par le client HTTP : `axios` brut pour `useBackendWarmup` (pas encore authentifié — passer par `apiClient` déclencherait la logique de refresh de l'intercepteur sur les 401 attendus, masquant le vrai signal), `apiClient` pour `useServicesWarmup` (déjà authentifié). Toute réponse applicative (401 inclus) vaut "prêt" ; seuls 429/502/503/504/absence de réponse font réessayer. **Architecture initiale (réveiller seulement auth/gateway sur Login, les 5 métier seulement après connexion) rejetée après un vrai test utilisateur bloquant** : ça créait un DEUXIÈME écran d'attente séparé juste après une connexion pourtant réussie (page Locataires bloquée avec 17 erreurs 429/502 en console), et forçait à réveiller chaque service à la main en ouvrant son URL Render une par une. `useBackendWarmup` réveille maintenant les 6 chemins en parallèle dès Login/RegisterPage ; le bouton reste bloqué jusqu'à ce que TOUT soit prêt, avec `hasTimedOut` pour afficher un message explicite plutôt qu'un déblocage silencieux qui renvoie dans la boucle 429/compte-à-rebours du point précédent. **Le plafond de 60s (puis 150s pour les services métier) s'est révélé trop court en usage réel** : un service Spring Boot (JVM) cold-starte jusqu'à ~2 min sur ce tier gratuit (log réel : `Started AuthServiceApplication in 119.901 seconds`), et une chaîne à plusieurs sauts (Gateway -> auth) cumule ces délais plutôt que les paralléliser — porté à 180s pour les deux hooks. `main.tsx` complète en désactivant les relances automatiques de React Query sur un 429 (`retry: (n, err) => n < 2 && !isRateLimited(err)`), pour ne pas concurrencer ce réveil proactif. **L'intervalle de retry initial de 4s s'est lui aussi révélé contre-productif** : Gateway confirmée saine (démarrage propre en 50s, routes correctes vers les 6 services) et une visite directe d'un service endormi le réveillait de façon fiable en 1-2 min, mais les tentatives automatiques (4s × 5-6 chemins en parallèle) ne finissaient jamais par aboutir même après plusieurs minutes. Hypothèse retenue : ce rythme entretenait en continu la protection anti-abus de Render (429) au lieu de laisser le réveil se dérouler sans concurrence — porté à 15s, réduisant le volume de requêtes d'environ 75% sur la fenêtre de 180s.
