...
- **Réveil silencieux de TOUS les services (auth/gateway + les 5 métier) dès l'arrivée sur Login/RegisterPage (`useBackendWarmup`, `src/lib/useBackendWarmup.ts`), avec `useServicesWarmup`/`RequireAuth.tsx` en filet de sécurité post-connexion.** Logique partagée dans `src/lib/useWarmup.ts` (`isWarmingUp`/`hasTimedOut`), paramétrée par le client HTTP : `axios` brut pour `useBackendWarmup` (pas encore authentifié — passer par `apiClient` déclencherait la logique de refresh de l'intercepteur sur les 401 attendus, masquant le vrai signal), `apiClient` pour `useServicesWarmup` (déjà authentifié). Toute réponse applicative (401 inclus) vaut "prêt". **Une seule requête par chemin, avec un délai d'attente de 180s — pas de boucle de retry.** Ce n'est pas le design de départ : plusieurs itérations ont été nécessaires, chacune motivée par un vrai test bloquant côté utilisateur, pas par anticipation :
  1. Réveiller seulement auth/gateway sur Login et les 5 métier seulement après connexion créait un DEUXIÈME écran d'attente séparé juste après une connexion pourtant réussie (page Locataires bloquée, 17 erreurs 429/502) → tout démarre maintenant en parallèle dès Login/RegisterPage.
  2. Un plafond de 60s (puis 150s) était trop court : un service Spring Boot (JVM) cold-starte jusqu'à ~2 min sur ce tier gratuit (log réel : `Started AuthServiceApplication in 119.901 seconds`), et une chaîne à plusieurs sauts (Gateway -> auth) cumule ces délais plutôt que les paralléliser → porté à 180s.
  3. Un intervalle de retry de 4s puis 15s ne suffisait toujours pas : Gateway confirmée saine (démarrage propre en 50s, routes correctes vers les 6 services) et une visite manuelle directe réveillait un service de façon fiable en 1-2 min, mais les tentatives automatiques répétées n'aboutissaient jamais même après plusieurs minutes — alors même que les logs montraient Render répondre 502 (sa propre page d'erreur) en quelques secondes, donc pas de connexion tenue ouverte à attendre plus longtemps. Hypothèse retenue : des requêtes automatisées répétées ressemblent à du trafic abusif pour la protection anti-abus de Render et prolongent le blocage → passé à une seule requête par chemin (délai d'attente 180s, pas de nouvelle tentative), reproduisant le comportement qui a fonctionné manuellement.

  `main.tsx` complète en désactivant les relances automatiques de React Query sur un 429 (`retry: (n, err) => n < 2 && !isRateLimited(err)`), pour ne pas concurrencer ce réveil.
