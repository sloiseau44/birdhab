...
- [ ] **Déploiement Render (`render.yaml`)** : en cours de premier déploiement réel par
      l'utilisateur (7 des 8 services live au dernier point, `birdhab-document` en cours
      de résolution). Plusieurs écarts déjà trouvés et corrigés grâce à ce test réel, dont
      l'abandon de Cloudflare R2 (carte bancaire exigée) au profit de Backblaze B2, et un
      vrai bug de code (nom de bucket MinIO codé en dur) — voir « Décisions actées ».
