...
- [ ] **Déploiement Render (`render.yaml`)** : écrit et vérifié contre la documentation
      Render, mais jamais déployé pour de vrai (pas de compte Render/Neon/Cloudflare
      disponible) — à tester et corriger au premier déploiement réel.
...
| Déploiement (instance permanente) | **`render.yaml` (Render, tier gratuit) + Postgres externe (Neon) + stockage externe (Cloudflare R2)** | Motivé par le portail locataire à venir : une instance qui doit rester joignable même quand le PC du propriétaire est éteint, donc pas le bundle Docker local. Postgres et MinIO restent hors de Render car son tier gratuit supprime sa base après 44 jours et n'autorise aucun disque persistant. Autre limite découverte : un service web gratuit Render ne peut *recevoir* aucun trafic réseau privé (seulement en émettre), donc la Gateway route vers les 6 services via leurs URL publiques `*.onrender.com` plutôt qu'un nom d'hôte interne. `render.yaml` n'accepte pas de `--build-arg`, d'où un `Dockerfile` par service backend plutôt que le générique. **Non déployé de bout en bout** (pas de compte Render/Neon/Cloudflare disponible pour tester réellement) — vérifié uniquement contre la documentation officielle de Render, contrairement au bundle Docker local qui a été testé dans un vrai navigateur. Deux vrais trous déjà remontés par un test réel (version Postgres Neon non précisée, comptes à créer non explicités) et corrigés — voir aussi la ligne « Assistant de déploiement » ci-dessous. |
