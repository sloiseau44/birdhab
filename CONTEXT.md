...
| Réveil en cascade (502 générique Render) | **`useBackendWarmup` : une seule requête (pas de retry) vers TOUS les services (auth/gateway + les 5 métier) dès l'arrivée sur Login/RegisterPage, délai d'attente 180s, bouton "Préparation du service…" désactivé jusqu'à ce que tout réponde (401 inclus)** (`hasTimedOut` sinon, message explicite) ; `useServicesWarmup`/`RequireAuth.tsx` en filet de sécurité post-connexion ; logique partagée dans `src/lib/useWarmup.ts` | Plusieurs itérations motivées chacune par un vrai test bloquant, pas par anticipation : (1) réveiller seulement auth/gateway sur Login et les 5 métier après connexion créait un deuxième écran d'attente séparé (page Locataires bloquée, 17 erreurs 429/502) → tout démarre en parallèle dès Login ; (2) un plafond de 60s puis 150s était trop court, un service Spring Boot cold-starte jusqu'à ~2 min sur ce tier (log réel : `Started AuthServiceApplication in 119.901 seconds`) → porté à 180s ; (3) un intervalle de retry de 4s puis 15s n'aboutissait toujours jamais alors qu'une visite manuelle directe réveillait un service en 1-2 min, malgré une Gateway confirmée saine et des URLs correctes — hypothèse retenue : des requêtes automatisées répétées ressemblent à du trafic abusif pour la protection anti-abus de Render et prolongent le blocage → passé à une seule requête par chemin, reproduisant le comportement qui a fonctionné manuellement. `main.tsx` désactive aussi les relances automatiques de React Query sur un 429. |
