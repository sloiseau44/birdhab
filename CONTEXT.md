...
| Réveil des services métier après connexion | **`useServicesWarmup` appelé depuis `RequireAuth.tsx`** (`frontend/src/lib/`) : ping en parallèle de `property`/`tenant`/`lease`/`payment`/`document`, écran "Réveil des services…" jusqu'à ce que tous répondent ou 60s ; `main.tsx` désactive en plus les relances automatiques de React Query sur un 429 | `useBackendWarmup` réveille auth/gateway mais pas les autres services, chacun dormant indépendamment sur Render. Repéré via une vraie capture : page Locataires bloquée sur "Chargement…", 17 erreurs 429/502 en console (une par requête), aggravées par les relances automatiques de React Query qui martelaient des services encore endormis. |
