...
- [ ] **Déploiement Render (`render.yaml`)** : les 8 services sont live, l'inscription
      fonctionne de bout en bout après correction du SNI Nginx manquant vers la Gateway en
      HTTPS. Plusieurs écarts trouvés et corrigés grâce à ce test réel : version Postgres
      Neon non précisée, comptes à créer non explicités, abandon de Cloudflare R2 (carte
      bancaire exigée) pour Backblaze B2, nom de bucket MinIO codé en dur, SNI manquant
      vers la Gateway — voir « Décisions actées ». Reste à confirmer un usage prolongé sans
      nouvel écart.
- [ ] **Portail locataire** : pas commencé. Prochain chantier demandé, à démarrer par
      connexion + accès aux documents (voir « Décisions actées »).
...
| Déploiement (instance permanente) | **`render.yaml` (Render, tier gratuit) + Postgres externe (Neon) + stockage externe (Backblaze B2)** | Motivé par le portail locataire à venir : une instance qui doit rester joignable même quand le PC du propriétaire est éteint, donc pas le bundle Docker local. Postgres et le stockage restent hors de Render car son tier gratuit supprime sa base après 44 jours et n'autorise aucun disque persistant. Autre limite découverte : un service web gratuit Render ne peut *recevoir* aucun trafic réseau privé (seulement en émettre), donc la Gateway route vers les 6 services via leurs URL publiques `*.onrender.com` plutôt qu'un nom d'hôte interne. `render.yaml` n'accepte pas de `--build-arg`, d'où un `Dockerfile` par service backend plutôt que le générique. **Les 8 services sont live et l'inscription fonctionne de bout en bout** — plusieurs vrais trous remontés et corrigés grâce à ce test réel : version Postgres Neon non précisée, comptes à créer non explicités, **Cloudflare R2 (premier choix de stockage) abandonné pour Backblaze B2** (carte bancaire exigée par R2 dès l'activation, même à 0€), `JWT_SECRET` collé tronqué sur un service (Spring refuse une clé HMAC < 256 bits), bouton de progression de l'assistant bloqué par une détection de clic peu fiable, nom de bucket MinIO codé en dur (devenu `MINIO_BUCKET`), et **SNI manquant vers la Gateway en HTTPS** (`proxy_ssl_server_name on` requis dans `frontend/nginx.conf.template`, invisible en local où l'amont est en `http://` — voir aussi les lignes « Assistant de déploiement » et « Réveil Render (429) » ci-dessous). |
| Réveil Render (429 "hibernate-rate-limited") | **`isRateLimited` + `useCooldown` sur `LoginPage`/`RegisterPage`** (`frontend/src/lib/`) : compte à rebours de 60s qui désactive le bouton plutôt qu'un message d'échec | Observé en usage réel : un service gratuit endormi répond 429 pendant son réveil, et recliquer pendant ce délai ne fait que le prolonger côté Render. Bloquer physiquement le bouton plutôt que compter sur la discipline de l'utilisateur. À reproduire sur d'autres pages si le même 429 s'y observe. |
| Réveil en cascade (502 générique Render) | **`useBackendWarmup` : ping silencieux vers `/auth/me` dès l'arrivée sur Login/RegisterPage, bouton "Préparation du service…" désactivé jusqu'à une vraie réponse (401 inclus) ou 180s** (`hasTimedOut` sinon, message explicite) | Le 429 ci-dessus ne suffisait pas : plusieurs services gratuits Render se réveillent indépendamment (frontend -> gateway -> auth), et soumettre le formulaire avant que toute la chaîne soit chaude produit un 502 générique Render (page d'erreur Render elle-même, pas une erreur applicative) même après avoir respecté le délai du 429. Pousser le réveil au tout début de la visite plutôt que de le découvrir à la soumission. **Le plafond de 60s s'est révélé trop court en usage réel** (utilisateur coincé dans la boucle 429/compte-à-rebours) : Gateway doit démarrer avant même de transmettre à auth, qui démarre à son tour — jusqu'à ~2 min chacun (log réel : `Started AuthServiceApplication in 119.901 seconds`), pire cas proche de la somme des deux. Porté à 180s. |
| Réveil des services métier après connexion | **`useServicesWarmup` appelé depuis `RequireAuth.tsx`** (`frontend/src/lib/`) : ping en parallèle de `property`/`tenant`/`lease`/`payment`/`document`, écran "Réveil des services…" jusqu'à ce que tous répondent ou 150s (`hasTimedOut` sinon, message + bouton "Réessayer") ; `main.tsx` désactive en plus les relances automatiques de React Query sur un 429 | `useBackendWarmup` réveille auth/gateway mais pas les autres services, chacun dormant indépendamment sur Render. Repéré via une vraie capture : page Locataires bloquée sur "Chargement…", 17 erreurs 429/502 en console (une par requête), aggravées par les relances automatiques de React Query qui martelaient des services encore endormis. **Le plafond de 60s s'est révélé trop court en usage réel** (page Biens retombée sur "Une erreur est survenue" après le délai) : ces 5 services cold-startent pour la première fois en parallèle (jamais sollicités depuis le login, contrairement à auth/gateway) — porté à 150s, avec un état d'échec explicite plutôt qu'un silence qui laisse la page sous-jacente échouer à son tour. |
| Connexions Neon mortes dans le pool Hikari (login/inscription en échec) | **`spring.datasource.hikari.max-lifetime: 180000` + `maximum-pool-size: 5`** dans les 6 `application.yml` backend | Diagnostiqué via de vrais logs `birdhab-auth` : rafale de `Failed to validate connection ... this connection has been closed` sur les 10 connexions du pool. Neon suspend son instance après quelques minutes d'inactivité et ferme les connexions TCP sans prévenir Hikari (`max-lifetime` par défaut 30 min, bien plus long) ; la première requête après une pause tombait sur des connexions mortes, provoquant des échecs de connexion/inscription pris à tort pour de vrais rejets (compte pourtant présent en base). `max-lifetime` réduit sous le délai de suspension Neon force un recyclage proactif. |
| Assistant de déploiement (`setup-wizard.html`) | **Page HTML autonome (zéro dépendance) en complément du README, pas à sa place** | Plutôt que de continuer à corriger le README à chaque trou découvert en testant, un assistant qui guide les 7 étapes, calcule les valeurs dérivées et prépare un récapitulatif copier-coller par service. Rien n'est envoyé nulle part (état en `localStorage` uniquement). Testé réellement dans un navigateur (pas juste relu), a révélé deux bugs corrigés avant livraison : perte de focus clavier à chaque frappe (le rendu reconstruit toute la page pour les calculs dérivés, sans quoi un nœud DOM recréé n'est jamais focus) et un bouton "suivant" bloqué en permanence désactivé (attribut `disabled` posé même avec une valeur `null`, ce qui désactive un bouton HTML quelle que soit la valeur). |
| Portail locataire (prochain chantier) | **Démarrer par connexion + accès aux documents (`auth` + `document`), la messagerie ensuite et à part** | Rôle `TENANT` déjà présent en placeholder dans `RoleName` (`auth`) mais jamais assigné ni utilisé pour restreindre un accès — vrai chantier neuf, pas un ajustement. La messagerie impliquerait un nouveau microservice (`messaging`), une décision séparée à ne pas mélanger avec la connexion/les documents. |
