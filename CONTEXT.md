...
- [ ] **Déploiement Render (`render.yaml`)** : écrit et vérifié contre la documentation
      Render, mais jamais déployé pour de vrai (pas de compte Render/Neon/Backblaze
      disponible) — à tester et corriger au premier déploiement réel. Plusieurs écarts
      déjà trouvés et corrigés en test réel par l'utilisateur, dont l'abandon de
      Cloudflare R2 (carte bancaire exigée) au profit de Backblaze B2.
...
| Déploiement (instance permanente) | **`render.yaml` (Render, tier gratuit) + Postgres externe (Neon) + stockage externe (Backblaze B2)** | Motivé par le portail locataire à venir : une instance qui doit rester joignable même quand le PC du propriétaire est éteint, donc pas le bundle Docker local. Postgres et le stockage restent hors de Render car son tier gratuit supprime sa base après 44 jours et n'autorise aucun disque persistant. Autre limite découverte : un service web gratuit Render ne peut *recevoir* aucun trafic réseau privé (seulement en émettre), donc la Gateway route vers les 6 services via leurs URL publiques `*.onrender.com` plutôt qu'un nom d'hôte interne. `render.yaml` n'accepte pas de `--build-arg`, d'où un `Dockerfile` par service backend plutôt que le générique. **Non déployé de bout en bout** (pas de compte Render/Neon/Backblaze disponible pour tester réellement) — vérifié uniquement contre la documentation officielle de Render, contrairement au bundle Docker local qui a été testé dans un vrai navigateur. Plusieurs vrais trous déjà remontés par un test réel et corrigés : version Postgres Neon non précisée, comptes à créer non explicités, puis **Cloudflare R2 (premier choix de stockage) abandonné pour Backblaze B2** — R2 exige une carte bancaire dès l'activation de son abonnement, même pour rester à 0€, ce que Backblaze ne demande pas. Voir aussi la ligne « Assistant de déploiement » ci-dessous. |
