...
| Réveil en cascade (502 générique Render) | **`useBackendWarmup` : ping silencieux vers TOUS les services (auth/gateway + les 5 métier) dès l'arrivée sur Login/RegisterPage, bouton "Préparation du service…" désactivé jusqu'à ce que tout réponde (401 inclus) ou 180s** (`hasTimedOut` sinon, message explicite) ; `useServicesWarmup`/`RequireAuth.tsx` en filet de sécurité post-connexion ; logique partagée dans `src/lib/useWarmup.ts` | Le 429 ci-dessus ne suffisait pas : plusieurs services gratuits Render se réveillent indépendamment, et soumettre le formulaire avant que toute la chaîne soit chaude produit un 502 générique Render même après avoir respecté le délai du 429. **Architecture initiale (auth/gateway seulement sur Login, les 5 métier seulement après connexion) rejetée après un vrai test bloquant** : ça créait un deuxième écran d'attente juste après une connexion réussie (page Locataires bloquée, 17 erreurs 429/502), et forçait à réveiller chaque service à la main. Tout démarre maintenant en parallèle dès Login/RegisterPage. **Les plafonds initiaux (60s puis 150s) se sont révélés trop courts** : un service Spring Boot cold-starte jusqu'à ~2 min sur ce tier gratuit (log réel : `Started AuthServiceApplication in 119.901 seconds`), et une chaîne à plusieurs sauts (Gateway -> auth) cumule ces délais — porté à 180s partout. **L'intervalle de retry de 4s s'est lui aussi révélé contre-productif** : Gateway confirmée saine et URLs correctes, mais les tentatives automatiques n'aboutissaient jamais alors qu'une visite directe réveillait le service en 1-2 min — hypothèse retenue, ce rythme entretenait la protection anti-abus 429 de Render en continu ; porté à 15s. `main.tsx` désactive aussi les relances automatiques de React Query sur un 429. |
